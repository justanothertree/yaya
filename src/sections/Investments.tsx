...
function explainInvestmentsError(err: unknown) {
  ...
  if (
    /function\s+public\.(get_family_accounts|get_executed_trades|get_allocations|insert_trade_even_split)/i.test(
      msg,
    )
  ) {
    return [
...
function parseAccountsCsv(text: string): string[] {
  ...
  const names = body.map((r) => (r[0] ?? '').trim()).filter(Boolean)
...
function parseTradesCsv(text: string): ParsedTradeRow[] {
  ...
  for (const row of body) {
    ...
    const unitsRaw = hasHeader
      ? get(row, 'units_acquired', 'units', 'quantity', 'qty')
      : (row[1] ?? '').trim()
  ...
  async function devDeleteTestRows() {
    ...
    try {
      ...
      logCallStart({
        ...
        user_id: uid,
        filter:
          'user_id=... AND (family_account_id in DEV accounts OR executed_trade_id in DEV trades)',
      })

      const accountIds = accounts
        .filter((a) =>
          String(
            (a as unknown as Record<string, unknown>).display_name ?? a.account_name ?? '',
          ).startsWith('[DEV]'),
        )
        .map((a) => a.id)
      const tradeIds = trades
        .filter((t) =>
          String(
            (t as unknown as Record<string, unknown>).asset_symbol ?? t.symbol ?? '',
          ).startsWith('DEV'),
        )
        .map((t) => t.id)
    ...
    <section className="grid" style={{ gap: '1rem' }}>
      ...
      {managementToolsEnabled && userId && (
        <article className="card" style={{ display: 'grid', gap: 12 }}>
          ...
          <p className="muted" style={{ margin: 0 }}>
            Uses <code>supabase.rpc(...)</code> and scopes to your <code>user_id</code>. Intended
            for your signed-in account.
          </p>
          <p className="muted" style={{ margin: 0 }}>
            Recommended order: <strong>1)</strong> Create family account → <strong>2)</strong>{' '}
            Create executed trade → <strong>3)</strong> Create allocation. The allocation IDs
            auto-fill from the last created rows.
          </p>
          ...
          <div style={{ display: 'grid', gap: 10 }}>
            ...
            <div style={{ display: 'grid', gap: 8 }}>
              ...
              <span className="muted" style={{ fontSize: 12 }}>
                Creates a <code>finance.executed_trades</code> row. “Units” ={' '}
                <code>units_acquired</code>.
              </span>
              <span className="muted" style={{ fontSize: 12 }}>
                Even Split Trade inserts the trade and auto-allocates units equally across all
                family accounts.
              </span>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                <input
            ...
            <div style={{ display: 'grid', gap: 8 }}>
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                <button
                  className="btn"
                  onClick={() => void bulkImportAccounts()}
                  disabled={bulkBusy}
                >
            ...
            <div style={{ display: 'grid', gap: 8 }}>
              ...
              <span className="muted" style={{ fontSize: 12 }}>
                <code>even_split</code>: true/false. If true, trade auto-allocates evenly across all
                family accounts.
              </span>
              <textarea
                value={bulkTradesCsv}
