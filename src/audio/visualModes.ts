...
function hue(ink: Ink, t: number, alpha = 1): string {
  /**
   * ⚠️ MORPHING IS NOT DONE HERE, and for one commit it was. Sliding the sample position
   * along the ramp moved the colours without the PALETTE changing — Ember stayed Ember, merely
   * cycling, which reads as the picture flickering rather than as one palette becoming another.
   * The tour lives in palettes.ts and arrives as different `stops`, so this never has to know it
   * is happening.
   */
