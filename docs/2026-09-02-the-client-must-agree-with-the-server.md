...
It goes wrong in three distinct ways, and all three turned up:
...
| ------------------------ | --------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| `save_my_profile_blocks` | block type allowlist, size, visibility, `length(config) > 16000`, 20 blocks | ✔ after today, except `art`, which is pending the migration and now fails gracefully |
| `save_site_content`      | admin, id, object, `length(p_doc::text) > 40000`                            | ✔ client stops at 36000                                                              |
| `set_my_profile_look`    | theme in 3, flair/backdrop `^[a-z][a-z0-9_-]*$` ≤ 40, palette 6-digit hex   | ✔ ids all lowercase; palette canonical after today                                   |
